...
describe('Hooch Tuples', function () {
  ...
  describe('ability checking', function () {
    before(function () {
      hooch.permit({
        activity: 'test.number.allowed',
        forItem: 'tuple.number',
      ...
      hooch.permit({
        activity: 'test.string.allowed',
        forItem: 'tuple.string',
        givenThat: (user, item, activity) => {
          return true;
        },
      });
    ...
    it('should be able to be matched in an ability check with numbers', function (done) {
      hooch
        .allow({
          user: 'user',
          isAllowedTo: 'test.number.allowed',
          forItem: hooch.tuple('tuple.number', 1),
        })
        .then((res) => {
          assert(res);
          done();
    ...
    it('should be able to be matched in an ability check with strings', function (done) {
      hooch
        .allow({
          user: 'user',
          isAllowedTo: 'test.string.allowed',
          forItem: hooch.tuple('tuple.string', 'kalle'),
        })
        .then((res) => {
          assert(res);
          done();
    ...
    it('should reject unmatched tuples', function (done) {
      hooch
        .allow({
          user: 'user',
          isAllowedTo: 'test.string.allowed',
          forItem: hooch.tuple('tuple.string.unmatched', 'kalle'),
        })
        .then((res) => {
          assert(false);
        })
